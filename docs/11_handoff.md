...
- ✅ **칩 생존 fix(2026-06-30, D-124)**: 플레이그라운드 실측서 호스트 LLM이 "눌러서 이어가기" 푸터·칩·공유카드를 재작성 과정에 드롭 확인 → kpass D-031/D-033 포팅(`footer.ts`에 조합 LLM **명시 지시문** + `💬 다음으로 물어보세요`, 백틱·UI-메타 제거). getTodayFortune 본문 이어가기 줄(D-122) 제거. 테스트 74. ✅**재검증 완료**(d74e75b, 플레이그라운드): 호스트가 칩 4개 + 공유카드 + "나도 보기" CTA surface, 지시문 미노출. → 칩 여정·바이럴 루프가 카카오 AI채팅 surface에서 실동작 확인(kpass D-033 TODO 클리어).
