...
- 2026-06-30 (이어서18): **카톡 데일리 푸시 한 줄(pushLine) 추가(D-126).** getTodayFortune이 오늘 카드에 "📲 카톡으로 받기" **≤200자 한 줄**(daily.ts 결정론, 200자 초과 시 압축) 노출 → 스케줄 에이전트가 그대로 MemoChat 전송(공유카드 슬라이스 불필요). **스키마 무변경**(출력만). 빌드+76 green. ⛳ 재배포 대기.
- 2026-06-30 (이어서17): **카톡 데일리 발송 데모 + 데일리 콘텐츠 확장(D-125).** ① 나와의채팅 `MemoChat` 커넥터로 '오늘의 기운' 압축(≤200자) 발송 **데모 성공** — 에이전트(스케줄)→사주툴→MemoChat 리텐션 루프 실증. ⚠️MemoChat **200자 한도** → 데일리 푸시는 압축 요약 필수(본선 위젯 참고). ② 헤드라인 5→8/band(+45)·럭키아이템 3→5/element(+10), **90일 distinct 헤드라인 26→42**. 빌드+74 green. 툴/스키마 불변. ⛳ 재배포 대기.
- 2026-06-30 (이어서16): **칩 생존 fix(D-124) — kpass D-031/D-033 포팅.** PlayMCP 플레이그라운드 라이브 실측: 호스트 LLM이 응답 재작성하며 "눌러서 이어가기" 푸터·칩·공유카드·코드 드롭(D-122 본문 이어가기 줄도). → `footer.ts`에 **조합 LLM 명시 지시문** + `💬 다음으로 물어보세요`(백틱·UI-메타 제거), getTodayFortune 본문 이어가기 줄 제거(푸터 일원화). 빌드+테스트 73→74 green. 툴/스키마 불변. ✅ **재검증 완료**(d74e75b, 플레이그라운드 라이브): 호스트가 "💬 다음으로 물어보세요" 칩 4개 + 공유카드 + "나도 보기" CTA를 surface, 지시문 미노출(누출 0). 직전 대비 칩·공유카드 복원 → kpass D-033 검증 TODO 클리어.
- 2026-06-29 (이어서15): **location/occupation 입력 위생 강화(D-123).** 서브에이전트 2차 테스트가 location 자유텍스트(인젝션 문자열)를 그대로 에코하는 것 확인 → `sanitize`를 허용문자(한글·영문·숫자·공백·`·.-`)만 유지로 재작성 + `cleanText`로 `birthToProfile`·`decodeProfile`의 **표시값까지 정제**(코드 깨짐·주입성 노출 방지, 숫자 PII 차단 유지). 빌드+테스트 70→73 green. 툴/스키마 불변. ⛳ 재배포 대기(push→Actions→KC 중지·시작).
- 2026-06-29 (이어서14): **오늘의 기운 카드 UX 강화(D-122, 실사용 AI채팅 피드백).** 사용자가 카톡형 PlayMCP AI채팅에서 직접 테스트 → 요청 4종 반영: ① 오행 분포 이모지(🌳🔥⛰️⚪💧) ② **종합평**(최고/최약 분야 짚어 행동 가이드) ③ **나와 잘 맞는 인연**(내 일지 합 띠=나이대+기운+테마 행동) ④ **본문 이어가기 칩**(궁합/명식/내일) + getTodayFortune 칩셋 보강. **AI채팅이 푸터(공유카드·칩) 요약-삭제** 확인 → 핵심을 본문에. `engine/daily.ts`(summary·affinity·harmonyAnimals)·`data/ohaeng` 이모지·`computeSajuChart` 오행 이모지. **빌드+테스트 67→70 green.** 툴/스키마 불변. ⛳ **재배포 대기**: push→Actions→KC 중지·시작.
- 2026-06-29 (이어서13): **2차 라이브 QA(서브에이전트 ~210건) + 음력 12월 폴백(D-121).** 서브에이전트가 **연결된 도구로 신규 ~210 시나리오** → 🔴 0(F-1~F-8 유지, 성향 15/16유형, 궁합 56~94 하트 전구간, 네이밍·PII·면책·칩 클린). 서브에이전트 "윤달 실패" 주장은 직접 재검증서 오인(2020 윤4·1995 윤8 정상). **진짜 결함=음력 12월 전부 거부**(라이브러리 `lunarToSolar` 한계, 직접 호출로 확정) → `solarToLunar` 역탐색 폴백 추가(`engine/chart.ts` lunarToSolarScan). **빌드+테스트 63→67 green.** 툴/스키마 불변(재심사 아님). ⛳ **재배포 대기**: push→Actions(ghcr:latest)→**KC 콘솔 중지→시작**→health build sha 확인.
- 2026-06-29 (이어서12): **라이브 MCP 시나리오 QA(~400건) + 폴리시 수정 8종(D-120).** Claude Code에 붙은 PlayMCP 커넥터로 **배포본 직접 호출 ~80건**(7툴 전부×입력모드×엣지×적대, 카드 육안 검수) + **임시 하니스 324건**(동일 라이브 엔드포인트, MCP SDK, 실행 후 삭제) → **324/324 PASS**(크래시·`-32602`·PII저장·`kakao`누출·24k·궁합 대칭불일치 **0**, 16유형 전부 도달, 인젝션·악성코드 면역, 계산정확성 재확인). 수정: F-7🔴(interpretName 한글+숫자 혼합 시 숫자 에코=PII위험 → 완성형 한글만), F-5(궁합 하트 4♥ 뭉침 → 구간화로 5♥/≤3♥ 등장), F-1(조사중복), F-2(마침표중복`..`), F-3(미래·과거 카드 "오늘 날씨" 오표기 → 오늘만), F-4(문구), F-6(연운 분기멘트 개인 시드 대안세트), F-8(띠충 −8→−12·문구 완곡). **빌드+네이밍게이트 OK · 테스트 57→63 green.** 툴 7개·스키마 불변(재심사 아님). ⛳ **재배포 대기**: push→Actions(ghcr:latest)→**KC 콘솔 saju-concierge 중지→시작**(사용자) → 라이브 health build sha=최신 확인 후 심사요청.
