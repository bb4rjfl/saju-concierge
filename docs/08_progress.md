...
- 2026-06-30 (이어서16): **칩 생존 fix(D-124) — kpass D-031/D-033 포팅.** PlayMCP 플레이그라운드 라이브 실측: 호스트 LLM이 응답 재작성하며 "눌러서 이어가기" 푸터·칩·공유카드·코드 드롭(D-122 본문 이어가기 줄도). → `footer.ts`에 **조합 LLM 명시 지시문** + `💬 다음으로 물어보세요`(백틱·UI-메타 제거), getTodayFortune 본문 이어가기 줄 제거(푸터 일원화). 빌드+테스트 73→74 green. 툴/스키마 불변. ⛳ **재배포 후 플레이그라운드 재테스트로 지시문 효과 확인 예정**(칩이 실제 surface되는지).
